...
void PololuSMC::init_port(){
    ...
    for (int i = 0; i < 4; i++){
        printf("    Starting motor %d...", i);
        smcs.push_back(SMCSerial(fd, i));
        printf("Success!\n");
    }
    printf("    Starting motor %d...", 5);
    smcs.push_back(SMCSerial(fd, 13));
    printf("Success!\n");
    printf("Initialization complete! %d motors are ready.\n", smcs.size());
}
void PololuSMC::exit_safe_start(int id){
    smcs[id].exit_safe_start();
}
uint16_t PololuSMC::get_error_status(int id){
    uint16_t status;
    int success = smcs[id].get_error_status(&status);
    return status;
}
uint16_t PololuSMC::get_serial_status(int id){
    uint16_t status;
    int success = smcs[id].get_variable(2, &status);
    return status;
}
uint16_t PololuSMC::get_limit_status(int id){
    uint16_t status;
    int success = smcs[id].get_variable(3, &status);
    return status;
}
uint16_t PololuSMC::get_uptime(int id){
    uint16_t time_low;
    uint16_t time_high;
...
uint16_t PololuSMC::get_temperatures(int id){
    uint16_t tempA;
    uint16_t tempB;
...
uint16_t PololuSMC::get_input_voltage(int id){
    uint16_t voltage;
    int success = smcs[id].get_variable(23, &voltage);
    return voltage;
}
uint16_t PololuSMC::get_current(int id){
    uint16_t current;
    int success = smcs[id].get_variable(44, &current);
    return current;
}
int16_t PololuSMC::get_current_speed(int id){
    int16_t speed;
    int success = smcs[id].get_current_speed(&speed);
    return speed;
}
int16_t PololuSMC::get_target_speed(int id){
    int16_t speed;
    int success = smcs[id].get_target_speed(&speed);
    return speed;
}
void PololuSMC::set_target_speed(int id, int16_t speed){
    int success = smcs[id].set_target_speed(speed);
}
void PololuSMC::stop_all(){
    for (int i = 0; i < smcs.size(); i++){
        smcs[i].stop_motor();
    }
}
void PololuSMC::brake_all(){
    for (int i = 0; i < smcs.size(); i++){
        smcs[i].set_target_speed(0);
    }
}
void PololuSMC::safe_start_all(){
    for (int i = 0; i < smcs.size(); i++){
        smcs[i].exit_safe_start();
    }
}
int PololuSMC::open_serial_port(){
